...
let oraclePreviewPage = 1;
let elasticPreviewPage = 1;
...
function setupWorkflowEventListeners() {
    ...
    const previewDataLoadBtn = document.getElementById('previewDataLoadBtn');
    const dataLoadElasticEnv = document.getElementById('dataLoadElasticEnv');
    const oraclePrevPageBtn = document.getElementById('oraclePrevPage');
    const oracleNextPageBtn = document.getElementById('oracleNextPage');
    const elasticPrevPageBtn = document.getElementById('elasticPrevPage');
    const elasticNextPageBtn = document.getElementById('elasticNextPage');
    ...
    if (previewDataLoadBtn) {
        previewDataLoadBtn.addEventListener('click', showDataPreview);
    }
    if (dataLoadElasticEnv) {
        dataLoadElasticEnv.addEventListener('change', handleDataLoadElasticEnvChange);
    }
    if (oraclePrevPageBtn) {
        oraclePrevPageBtn.addEventListener('click', () => {
            if (oraclePreviewPage > 1) {
                oraclePreviewPage--;
                fetchDataPreview();
            }
        });
    }
    if (oracleNextPageBtn) {
        oracleNextPageBtn.addEventListener('click', () => {
            oraclePreviewPage++;
            fetchDataPreview();
        });
    }
    if (elasticPrevPageBtn) {
        elasticPrevPageBtn.addEventListener('click', () => {
            if (elasticPreviewPage > 1) {
                elasticPreviewPage--;
                fetchDataPreview();
            }
        });
    }
    if (elasticNextPageBtn) {
        elasticNextPageBtn.addEventListener('click', () => {
            elasticPreviewPage++;
            fetchDataPreview();
        });
    }
...
function populateWorkflowTables(tables) {
    ...
    if (!tables || tables.length === 0) {
        container.innerHTML = `
            <div class="text-center text-muted py-5">
                ...
                <p>No tables, views, or synonyms found in this Oracle environment</p>
            </div>
        `;
        return;
    }
    ...
    tables.forEach((table, index) => {
        ...
        tableItem.dataset.type = type;
        const icon = type === 'VIEW' ? 'fa-eye' : type === 'SYNONYM' ? 'fa-link' : 'fa-table';
        const label = type === 'VIEW' ? 'Oracle View' : type === 'SYNONYM' ? 'Synonym' : 'Oracle Table';
        tableItem.innerHTML = `
            <div class="d-flex align-items-center">
...
function updateTableSelectionSummary() {
    ...
    if (count === 0) {
        summaryContainer.innerHTML = '<p class="text-muted">No objects selected yet</p>';
        loadBtn.disabled = true;
    } else {
        let summaryHTML = `<h6 class="text-primary">${count} Selection(s):</h6><ul class="list-unstyled">`;
        workflowData.selectedTables.forEach(table => {
            const type = workflowData.selectedTableTypes[table] || 'TABLE';
            const icon = type === 'VIEW' ? 'fa-eye' : type === 'SYNONYM' ? 'fa-link' : 'fa-table';
            summaryHTML += `<li><i class="fas ${icon} me-2 text-danger"></i>${table} <span class="badge bg-light text-dark">${type}</span></li>`;
        });
        summaryHTML += '</ul>';
...
async function fetchDataPreview() {
    ...
    try {
        ...
        const result = await response.json();
        if (result.success) {
            renderOraclePreview(result.oracle);
            renderElasticPreview(result.elastic);
        } else {
            showAlert('Preview failed: ' + (result.error || 'Unknown error'), 'danger');
        }
    } catch (err) {
...
function renderOraclePreview(data) {
    const table = document.getElementById('oraclePreviewTable');
    ...
    if (!data || !data.rows || data.rows.length === 0) {
        tbody.innerHTML = '<tr><td class="text-center text-muted">No data</td></tr>';
        document.getElementById('oraclePreviewInfo').textContent = '';
        document.getElementById('oraclePrevPage').disabled = oraclePreviewPage <= 1;
        document.getElementById('oracleNextPage').disabled = true;
        return;
    }
    ...
    const totalPages = Math.max(1, Math.ceil((data.total || 0) / previewPageSize));
    document.getElementById('oraclePreviewInfo').textContent = `Page ${oraclePreviewPage} of ${totalPages}`;
    document.getElementById('oraclePrevPage').disabled = oraclePreviewPage <= 1;
    document.getElementById('oracleNextPage').disabled = oraclePreviewPage >= totalPages;
}

function renderElasticPreview(data) {
    const table = document.getElementById('elasticPreviewTable');
    const thead = table.querySelector('thead');
    const tbody = table.querySelector('tbody');
    thead.innerHTML = '';
    tbody.innerHTML = '';
    ...
    if (!data || !data.hits || data.hits.length === 0) {
        tbody.innerHTML = '<tr><td class="text-center text-muted">No data</td></tr>';
        document.getElementById('elasticPreviewInfo').textContent = '';
        document.getElementById('elasticPrevPage').disabled = elasticPreviewPage <= 1;
        document.getElementById('elasticNextPage').disabled = true;
        return;
    }

    const headers = Object.keys(data.hits[0]);
    const headerRow = document.createElement('tr');
    headers.forEach(h => {
        const th = document.createElement('th');
        th.textContent = h;
    ...
    const totalPages = Math.max(1, Math.ceil((data.total || 0) / previewPageSize));
    document.getElementById('elasticPreviewInfo').textContent = `Page ${elasticPreviewPage} of ${totalPages}`;
    document.getElementById('elasticPrevPage').disabled = elasticPreviewPage <= 1;
    document.getElementById('elasticNextPage').disabled = elasticPreviewPage >= totalPages;
}
...
function removeRelationship(index) {
    ...
    // 🔄 Reindex field type overrides so mapping generation uses current relationship order
...
// 🔧 ENHANCED generateElasticsearchFieldsHTML with editing support
