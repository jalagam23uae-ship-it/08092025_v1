...
def get_table_columns(url: str, username: str, password: str, table_name: str,
                      schema: str = None, include_synonyms: bool = True,
                      include_views: bool = True):
    logger.info(include_views)
    logger.info(include_synonyms)
    """
    Get columns for a specific Oracle table, view, or synonym

    Args:
        url: Oracle database connection string or DSN
        username: Database username
        password: Database password
        table_name: Name of table/view/synonym
        schema: Optional schema name (defaults to current user)
        include_synonyms: Whether to search synonyms
        include_views: Whether to search views

    Returns:
        dict: Success status, columns list, and object type info
    """
    try:
        import oracledb
        ...
        # Parse connection string
        if '@' in url:
            connection_parts = url.split('@', 1)  # Split on first @ only
            dsn = connection_parts[1] if len(connection_parts) == 2 else url
        else:
            dsn = url
        ...
        with oracledb.connect(user=username, password=password, dsn=dsn) as connection:
            ...
            target_schema = schema.upper() if schema else username.upper()
            table_upper = table_name.upper()

            # First, try to find the object and determine its type
            object_info_query = """
            ...
            cursor.execute(object_info_query, (table_upper, target_schema))
            object_info = cursor.fetchone()

            if not object_info:
                # If not found in specified schema, try current user schema
                if schema:  # Only try if we specified a different schema
                    cursor.execute(object_info_query, (table_upper, username.upper()))
                    object_info = cursor.fetchone()

            if not object_info:
                # Try searching across accessible schemas
                accessible_query = """
                                   SELECT object_type, owner, object_name
                                   FROM all_objects
                                   WHERE object_name = :1 
                ...
                AND owner IN (
                    ...
                    UNION SELECT owner FROM all_tables WHERE owner != 'SYS'
                )
                                   ORDER BY CASE object_type
                                       WHEN 'TABLE' THEN 1
                                       WHEN 'VIEW' THEN 2
                                   ...
                                   END \
                                   """
                cursor.execute(accessible_query, (table_upper,))
                object_info = cursor.fetchone()

            if not object_info:
                return {
                    "success": False,
                    "message": f"Table/view/synonym '{table_name}' not found or not accessible",
                    "columns": [],
                    "object_type": None,
                    "schema": None
                }

            obj_type, obj_owner, obj_name = object_info

            # Handle synonyms by resolving to actual table/view
            if obj_type == 'SYNONYM':
                synonym_query = """
                                SELECT table_owner, table_name
                                FROM all_synonyms
                                WHERE synonym_name = :1 AND owner = :2 \
                                """
                cursor.execute(synonym_query, (obj_name, obj_owner))
                synonym_info = cursor.fetchone()

                if synonym_info:
                    obj_owner, obj_name = synonym_info
                else:
                    return {
                        "success": False,
                        "message": f"Could not resolve synonym '{table_name}'",
                        "columns": [],
                        "object_type": "SYNONYM",
                        "schema": obj_owner
                    }
            ...
            cursor.execute(columns_query, (obj_name, obj_owner))

            columns = []
            for row in cursor.fetchall():
                col_name, data_type, data_length, data_precision, data_scale, nullable, data_default, column_id = row

                # Format data type with precision/scale for numeric types
                ...
                columns.append({
                    ...
                    'scale': data_scale,
                    'nullable': nullable == 'Y',
                    'default_value': data_default.strip() if data_default else None,
                    'column_id': column_id
                })
        ...
        return {
            "success": False,
            "message": "oracledb module not installed. Install with: pip install oracledb",
            "columns": [],
            "object_type": None,
            "schema": None
        }
...
def get_table_columns_v1(url: str, username: str, password: str, table_name: str):
...
async def oracle_data_load(
    ...
    try:
        ...
        paged_query = f"{query} OFFSET {oracle_offset} ROWS FETCH NEXT {limit} ROWS ONLY"
        with oracledb.connect(
            ...
            try:
                ...
                # Handle ambiguous column names by falling back to client-side slicing
                if "ORA-00918" in str(db_err):
                    cursor.execute(query)
                    columns = [c[0] for c in cursor.description]
                    all_rows = cursor.fetchall()
                    rows = all_rows[oracle_offset:oracle_offset + limit]
                else:
                    raise
...
@app.post("/oracle/data-preview")
async def oracle_data_preview(
    oracle_env_id: int = Form(...),
    elastic_env_id: int = Form(...),
    index: str = Form(...),
    query: str = Form(...),
    ...
    """Preview Oracle query results and Elasticsearch index data side-by-side."""
    try:
        oracle_envs = get_oracle_environments()
        oracle_env = next((e for e in oracle_envs if e["id"] == oracle_env_id), None)
        if not oracle_env:
        ...
        return {
            "success": True,
            "oracle": {"rows": rows, "total": total_rows, "page": oracle_page},
            "elastic": {"hits": hits, "total": es_total, "page": elastic_page},
        }
    except Exception as e:
        return JSONResponse({"success": False, "error": str(e)}, status_code=500)
...
def get_table_columns(url: str, username: str, password: str, object_name: str):
    """Get columns for a specific Oracle table, view, or synonym"""
    try:
        import oracledb
        ...
        with oracledb.connect(user=username, password=password, dsn=dsn) as connection:
            ...
            # Resolve if synonym → base table/view
            cursor.execute("""
                           SELECT table_owner, table_name
                           FROM all_synonyms
                           ...
                           """, (object_name,))
            synonym = cursor.fetchone()
            if synonym:
                owner, resolved_name = synonym
            else:
                owner, resolved_name = (username.upper(), object_name.upper())

            # Now fetch columns from ALL_TAB_COLUMNS (works for tables & views)
            cursor.execute("""
                           SELECT column_name, data_type, data_length, nullable
                           FROM all_tab_columns
                           WHERE owner = :owner
                             AND table_name = :name
                           ORDER BY column_id
                           """, {"owner": owner, "name": resolved_name})
...
def build_terms_aggregation(field_name: str,field_type: str) -> dict:
    if '.' in field_name:
        ...
        print(field_type)
        field_name = field_name + ".keyword" if field_type == "text" else field_name
        print(field_name)
        return {
            "size": 0,
            "aggs": {
...
def extract_field_types(properties, prefix=""):
    field_types = {}
    for field, attrs in properties.items():
        field_path = f"{prefix}.{field}" if prefix else field

        if attrs.get("type") == "nested":
            # Handle nested types recursively
            nested_props = attrs.get("properties", {})
            field_types[field_path] = "nested"
            field_types.update(extract_field_types(nested_props, prefix=field_path))
        else:
            # Normal field
            field_types[field_path] = attrs.get("type")
    return field_types
...
def build_enhanced_elasticsearch_query(fields: Dict[str, Any], form_config: Dict, metadata: Dict) -> Dict:
    ...
    mapping = get_elasticsearch_mapping(form_config.get('host_url'), form_config.get('index_name'), form_config.get('username'), form_config.get('password'))

# Build comprehensive Elasticsearch query
    field_types = extract_field_types(mapping[form_config.get('index_name')]["mappings"]["properties"])
    ...
    try:
        ...
        if inner_field_list is not None and len(inner_field_list) > 0:
            ...
            print(fieldsv1)
            tempv3=build_query_v6(fieldsv1,"AND",form_config.get("index_name"),field_types,nested_field_list,inner_field_list)
            print(tempv3)
            print("......")
            ...
            print("*******************************************************")
            tempv1=build_query_v6(fields,"AND",form_config.get("index_name"),field_types)
            print(tempv1)
...
def resolve_field(field_name: str, field_types: Optional[Dict[str, str]]) -> str:
    """
    If the field type is 'text', return field_name.keyword
    Otherwise return the original field_name
    """
    if not field_types:
        raise ValueError("field_types must be a dictionary, not None")
...
def build_query_v6(
    ...
    for key, value in result.items():
        ...
        if isinstance(value, dict) and value.get("type") == "multi_value":
            ...
            mv_key = resolve_field(original_key,field_types)
...
def build_query_v7(
    ...
    for key, value in result.items():
        ...
        if isinstance(value, dict) and value.get("type") == "multi_value":
            ...
            # For text-ish exact matching across multiple values, default to keyword sub-field
            print(field_types.get(original_key))
            mv_key = resolve_field(original_key,field_types.get(original_key))
...
def transform_query_v6(input_data):
    ...
    if len(main_groups) == 0:
        ...
        output["query"]["operator"]=(main_groups[0].get("operator", "AND"))
        output["query"]["conditions"].extend(main_groups[0].get("conditions", []))
    else:
        # Multiple groups: nest additional groups inside first group
        first_group = main_groups[0]
        outer = {
